...
def Astar(start_state,flag):
   ...
   heapq.heappush(priority_que,(0,start_state,[],[])) # push into the queue a tuble containing (cost of start, start_state, path to start state, moves made )
   ...
   while (priority_que):
      ...
      for neighbor, move  in zip (neighbor_states, new_moves): # loop through all resulting states from swapping with neighbors
         ...
         new_cost=cost +1
         total_cost= new_cost + HelpingFns.calc_heuristic(neighbor,flag)
