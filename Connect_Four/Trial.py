...
def toggle_alpha_beta_pruning():
    """
    Cycle through the three states of Alpha-Beta Pruning.
    """
    global alpha_beta_pruning_state
    alpha_beta_pruning_state = (alpha_beta_pruning_state + 1) % 3  # Cycle through 0, 1, 2

def draw_toggle_button():
    """
    Draw the toggle button for Alpha-Beta Pruning with three states.
    """
    x, y, width, height = SCREEN_WIDTH - 120, 20, 120, 40  # Adjust width to accommodate 3 options
    ...
    # Draw the label for the toggle button
    label = font.render("Alpha-Beta Pruning", True, (0, 0, 0))
    screen.blit(label, (x - 220, y + 5))  # Adjust positioning as needed
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif SCREEN_WIDTH - 120 <= x <= SCREEN_WIDTH and 20 <= y <= 60:
